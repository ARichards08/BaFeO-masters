...
! Indices 4-6 hold the information of which surrrouding cell the interacting atom is in
allocate (distances(6, (N)*((N*27)-1)), stat=istat)
...
do i=1, N, 1
    do j=1, N*27, 1
        ...
        if (x_int/=0 .or. y_int/=0 .or. z_int/=0) then
            ...
            distances(:, counter)=[r, cell_ortho(7, i), n_cell_ortho(7, j)&
...
allocate (Fe_distances(6, (cell_mag_atoms)*((cell_mag_atoms*27)-1)), stat=istat)
...
do i=1, size(distances, 2), 1
    ...
    do j=1, size(cell, 2), 1
        if ((cell(7, j) == atom_i) .and. (trim(material_id(cell(5, j))) == 'Fe')) then
            counter=counter+1
        end if
    end do

    do j=1, size(cell, 2), 1
        if ((cell(7, j) == atom_j) .and. (trim(material_id(cell(5, j))) == 'Fe')) then
            counter=counter+1
        end if
    end do
...
do i=1, size(Fe_distances, 2), 1
    if ((Fe_distances(2, i) == 52.0_dp) .and. (Fe_distances(3, i) == 52.0_dp)) then
        print *, Fe_distances(:, i)
    end if
end do
