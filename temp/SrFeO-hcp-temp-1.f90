...
real(kind=dp), dimension(:, :), allocatable :: Fe_distances
real(kind=dp) :: a, b, c, x, y, z, t, r, r2, min_r, min_r2, min_r3, atom_i, atom_j
integer :: N_prim, N, istat, i, j, k, l, counter, nn, nnn, nnnn, x_prim_mul, y_prim_mul, x_unit, y_unit, prim_in_unit, atom_types
integer :: failcount, x_int, y_int, z_int, mag_atoms, n_mag_atoms
...
n_mag_atoms=mag_atoms*prim_in_unit
...
allocate (prim_cell(7, N_prim), stat=istat)
...
do l=1, N_prim, 1
    do x_unit=0, x_prim_mul-1, 1
        do y_unit=0, y_prim_mul-1, 1
            ...
            &, prim_cell(4, l), prim_cell(5, l), prim_cell(6, l), prim_cell(6, l)]
            counter=counter+1
        end do
    end do
end do
...
allocate (n_cell(7, N*27), stat=istat)
    ...
    do i=-x_prim_mul, x_prim_mul, x_prim_mul
        do j=-y_prim_mul, y_prim_mul, y_prim_mul
            do k=-1, 1, 1 
                ...
                &, cell(4, l), cell(5, l), cell(6, l), cell(7, l)]
                counter=counter+1
            end do
        end do
    end do
...
allocate (n_cell_ortho(7, N*27), stat=istat)
...
allocate (distances(3, (N)*((N*27)-1)), stat=istat)
...
do i=1, N, 1
    do j=1, N*27, 1
        ...
        if (x_int/=0 .or. y_int/=0 .or. z_int/=0) then
            ...
            distances(:, counter)=[r, cell_ortho(7, i), n_cell_ortho(7, i)]
            counter=counter+1
        else
...
allocate (Fe_distances(3, (n_mag_atoms)*((n_mag_atoms*27)-1)), stat=istat)
...
!    print *, atom_
    ...
    do j=1, size(cell, 2), 1
        if (((cell(7, j) == atom_i) .or. (cell(7, j) == atom_j)) .and. (material_id(cell(5, j)) == 'Fe')) then
            counter=counter+1
        else
!            print *, , counter
        end if
...
print *, k, l
print *, Fe_distances(:, size(Fe_distances, 2))



! Round distances to 4 dp
do i=1, size(distances, 2), 1
    distances(1, i)=real(nint(distances(1, i)*10000), kind=dp)*0.0001_dp
end do

! Find the smallest distance
min_r=distances(1, 1)
do i=1, size(distances, 2), 1
    if (distances(1, i) < min_r) min_r=distances(1, i)
end do

! Find number of nn 
nn=0
do i=1, size(distances, 2), 1
    if (distances(1, i) == min_r .and. distances(2, i)==2) nn=nn+1
end do
...
min_r2=0.0_dp
do i=1, size(distances, 2), 1
    if (distances(1, i) > min_r2) min_r2=distances(1, i)
end do
do i=1, size(distances, 2), 1
    if (distances(1, i) > min_r .and. distances(1, i) < min_r2) min_r2=distances(1, i)
end do

nnn=0
do i=1, size(distances, 2), 1
    if (distances(1, i) == min_r2 .and. distances(2, i)==2) nnn=nnn+1
end do
...
min_r3=0.0_dp
do i=1, size(distances, 2), 1
    if (distances(1, i) > min_r3) min_r3=distances(1, i)
end do
do i=1, size(distances, 2), 1
    if (distances(1, i) > min_r2 .and. distances(1, i) < min_r3) min_r3=distances(1, i)
end do

nnnn=0
do i=1, size(distances, 2), 1
    if (distances(1, i) == min_r3 .and. distances(2, i)==2) nnnn=nnnn+1
end do
...
do i=1, size(positions, 2)
    ...
    elseif (trim(wyckoff_id(atom_wyckoff)) == '2a') then
        ! (0,0,0), (0,0,1/2)
        cell(:, counter) = [0.0_dp, 0.0_dp, 0.0_dp, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [0.0_dp, 0.0_dp, 1.0_dp/2.0_dp, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
    elseif (trim(wyckoff_id(atom_wyckoff)) == '2b') then
        ! (0,0,1/4), (0,0,3/4)
        cell(:, counter) = [0.0_dp, 0.0_dp, 1.0_dp/4.0_dp, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [0.0_dp, 0.0_dp, 3.0_dp/4.0_dp, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
    elseif (trim(wyckoff_id(atom_wyckoff)) == '4f') then
        ! (1/3,2/3,z), (2/3,1/3,z+1/2), (2/3,1/3,-z), (1/3,2/3,-z+1/2)
        cell(:, counter) = [1.0_dp/3.0_dp, 2.0_dp/3.0_dp, z, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [2.0_dp/3.0_dp, 1.0_dp/3.0_dp, z+1.0_dp/2.0_dp, atom_element, atom_material, atom_wyckoff,&
        & real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [2.0_dp/3.0_dp, 1.0_dp/3.0_dp, -z, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
    ...
    elseif (trim(wyckoff_id(atom_wyckoff)) == '12k') then
        ...
        cell(:, counter) = [x, 2.0_dp*x, z, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [-2.0_dp*x, -x, z, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [x, -x, z, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [-x, -2.0_dp*x, z+1.0_dp/2.0_dp, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [2.0_dp*x, x, z+1.0_dp/2.0_dp, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [-x, x, z+1.0_dp/2.0_dp, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [2.0_dp*x, x, -z, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [-x, -2.0_dp*x, -z, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
    ...
    elseif (trim(wyckoff_id(atom_wyckoff)) == '4e') then
        ...
        cell(:, counter) = [0.0_dp, 0.0_dp, z, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [0.0_dp, 0.0_dp, z+1.0_dp/2.0_dp, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [0.0_dp, 0.0_dp, -z, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [0.0_dp, 0.0_dp, -z+1.0_dp/2.0_dp, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
    ...
    elseif (trim(wyckoff_id(atom_wyckoff)) == '6h') then
        ...
        cell(:, counter) = [x, 2.0_dp*x, 1.0_dp/4.0_dp, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [-2.0_dp*x, -x, 1.0_dp/4.0_dp, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [x, -x, 1.0_dp/4.0_dp, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [-x, -2.0_dp*x, 3.0_dp/4.0_dp, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [2.0_dp*x, x, 3.0_dp/4.0_dp, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
        cell(:, counter) = [-x, x, 3.0_dp/4.0_dp, atom_element, atom_material, atom_wyckoff, real(counter, kind=dp)]
        counter=counter+1
    end if
end do
print *, size(cell, 2), counter, cell(:, size(cell, 2))
